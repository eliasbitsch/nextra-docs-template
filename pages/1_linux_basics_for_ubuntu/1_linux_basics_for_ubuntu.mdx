...
- **`/bin`**: Contains binary executables (e.g., `cat`, `mkdir`, `ls`). Named for **BIN**aries.
...
cd ~
```
6. Print text on terminal with `echo`.
```bash filename="bash" copy
...
```
---
2. Try to display the calendar of the current month:
```bash filename="bash" copy
cal
```
...
```
---
4. Make the terminal print a message in <span style={{ color: 'green' }}>green</span>:
```bash filename="bash" copy
...
cd
```
Commands can be auto-completed inside the Shell.<br/>
...
```
2. Let's try `--help` with the `cat` command.
```bash filename="bash" copy
cat --help
```
...
And as our great headmaster Albus Dumbledore stated 🧙🏼‍♂️: <GlowingText>*"Help will always be given at Hogwarts, Harry, to those who ask for it."*</GlowingText>
...
cd /tmp
```
2. `touch first_file`: Creates an empty file named `first_file`.
```bash filename="bash" copy
touch first_file
```
3. `ls`: Lists the files and folders in the current directory.
```bash filename="bash" copy
ls
```
...
first_file
```
4. `mkdir first-folder`: Creates a directory named `first-folder`.
```bash filename="bash" copy
mkdir first-folder
```
5. `ls`: Lists the files and folders in the current directory.
```bash filename="bash" copy
ls
```
6. `touch first-folder/second_file`: Creates an empty file named `second_file` inside the `first-folder` directory.
```bash filename="bash" copy
touch first-folder/second_file
```
7. `ls first-folder/`: Lists the files inside the `first-folder` directory.
```bash filename="bash" copy
ls first-folder/
```
```bash filename="output"
...
8. `rm -rf first-folder first_file`: Removes the `first-folder` directory and the `first_file` file (files deleted with `rm` cannot be restored).
...
```
9. Now when you list the files and folders in the current directory, you should see that the `first-folder` directory and the `first_file` file are gone.
```bash filename="bash" copy
ls
```
