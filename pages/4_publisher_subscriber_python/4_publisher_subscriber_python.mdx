...
catkin_make
```
3. Source the workspace & check if the package is in the workspace:
```bash filename="bash" copy
...
1. Execute the following lines in `2 different terminals` with `[ctrl + b] and [Shift + 5]`:
...
<launch>
    <include file="$(find first_pkg)/launch/homework_simulation.launch"/>
...
One of the most difficult but important aspects of robotics is: *knowing how to translate your ideas and knowledge into real-world projects*. Usually in robotics projects: *nothing works like in theory*. Reality is much more complex and therefore you need tools to figure out what is going on and where the problem is.
...
Create a new node called `logger_test.py` in the `first_pkg`:
...
log_level = sys.argv[1]  # First arg is path to file
if log_dict[log_level]:
    LOGGER = log_dict[log_level]
else:
    LOGGER = rospy.DEBUG
...
4. Inspect the topic `/rosout` again starting the `move_turtlebot.launch` file.
```bash filename="bash" copy  
